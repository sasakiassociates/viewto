...
namespace ViewTo.Connector.Unity
{
  ...
  public class ViewToHub : MonoBehaviour
  {
    ...
    [SerializeField] StreamScriptable _stream;
    ...
    async UniTask AutoStart()
    {
      ...
      var client = new SpeckleUnityClient(_stream.Account);
      client.token = this.GetCancellationTokenOnDestroy();

      var commit = await client.CommitGet(_stream.Id, _stream.Commit.id);

      if (commit == null)
        return;

      var @base = await SpeckleOps.Receive(client, _stream.Id, commit.referencedObject);
      ...
      foreach (var obj in _speckleStudy.Objects)
      {
        ...
        switch (obj)
        {
          case VS.ContentReference o:
            ...
            await GetContentData(content, o, client, _stream.Id);
          ...
          case VS.ViewCloudReference o:
            ...
            var co = await ReceiveCommitWithData(client, _stream.Id, cloud.Reference.FirstOrDefault());
      ...
      if (_createCommit)
      {
        ...
        try
        {
          UniTask.Create(async () =>
          {
            ...
            var res = await SpeckleOps.Send(client, new Base { ["Data"] = _speckleStudy }, _stream.Id);

            await client.CommitCreate(new CommitCreateInput
            {
              objectId = res,
              message = $"{_study.ViewName} is complete! {mono.count}",
              branchName = _stream.IsValid() && _stream.Branch.Valid() ? _stream.Branch.name : "main",

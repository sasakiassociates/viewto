...
namespace ViewTo
{
  ...
  public static class Results
  {
    ...
    public static bool CheckAgainstString(this ResultType type, string stage)
    {
      if (!stage.Valid())
        return false;

      // previous version of organizing data types
      var nameToCompare = type switch
      {
        ResultType.Potential => "Target",
        ResultType.Existing => "Blocker",
        ResultType.Proposed => "Design",
        _ => throw new ArgumentOutOfRangeException(nameof(type), type, null)
      };

      var res = stage.ToUpper().Equals(nameToCompare.ToUpper());

      if (res)
        return true;

      // try other version
      nameToCompare = type switch
      {
        ResultType.Potential => "Potential",
        ResultType.Existing => "Existing",
        ResultType.Proposed => "Proposed",
        _ => throw new ArgumentOutOfRangeException(nameof(type), type, null)
      };

      return stage.ToUpper().Equals(nameToCompare.ToUpper());
    }

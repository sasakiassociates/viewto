...
namespace ViewTo.Helpers
{
	public class PlyHandle
	{
		...
		public static string Create(string file, string filename, CloudPoint[] points)
		{
			var writer = new BinaryWriter(new FileStream(file, FileMode.Create), Encoding.ASCII);
			//Write the headers for 3 vertices
			writer.Write(str2byteArray("ply\n"));
			writer.Write(str2byteArray("format binary_little_endian 1.0\n"));
			writer.Write(str2byteArray($"element vertex {points.Length}\n"));

			foreach (var p in Commander.BasicParam()) writer.Write(str2byteArray($"property float {p}\n"));
			...
			{
				writer.Write(float2byteArray((float)point.x));
				writer.Write(float2byteArray((float)point.y));
				writer.Write(float2byteArray((float)point.z));

				writer.Write(float2byteArray((float)point.xn));
				writer.Write(float2byteArray((float)point.yn));
				writer.Write(float2byteArray((float)point.zn));
			}
		...
		DataHeader ReadDataHeader(StreamReader reader)
		{
			...
			if (line != "format binary_little_endian 1.0")
				throw new ArgumentException(
					"Invalid data format ('" + line + "'). " + "Should be binary/little endian.");
		...
		static byte[] float2byteArray(float value) => BitConverter.GetBytes(value);

		static byte[] str2byteArray(string theString) => Encoding.ASCII.GetBytes(theString);

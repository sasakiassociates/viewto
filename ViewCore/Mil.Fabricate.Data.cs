	...
	public static partial class Mil
	{
		public static partial class Fabricate
		{
			public static class Data
			{
				...
				public static uint[] ValuesUint(int valueCount, Random rnd = null)
				{
					rnd ??= new Random();
				...
				public static int[] ValuesInt(int valueCount, Random rnd = null)
				{
					...
					for (var j = 0; j < values.Length; j++)
					{
						var bytes = new byte[4];
						rnd.NextBytes(bytes);
						values[j] = BitConverter.ToInt32(bytes, 0);
					}

					return values;
				}

				public static int[][] ValuesInt(int valueCount, int contentCount)
				{
					var rnd = new Random();

...
public static class Extensions
{
  ...
  ///     one <see cref="Content" /> marked as <see cref="ContentType.Potential" /> and one marked as
  ...
  {
    return study.Has<IViewCloud>() && study.Has<IViewer>() && study.Has(ContentType.Potential) && study.Has(ContentType.Existing);
  }
  ...
  public static void ContentCount(this IViewStudy obj, out int targetCount, out int existingCount, out int proposedCount)
  {
    ...
    if(obj != default(object))
    {
      ...
      targetCount = contents.Count(x => x.ContentType == ContentType.Potential);
